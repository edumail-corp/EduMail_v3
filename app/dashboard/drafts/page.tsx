...
export default function DraftsPage() {
  ...
  const draftEmails = useMemo(
    () => mockEmails.filter((mail) => mail.status === "Draft"),
    []
  );
  const [selectedId, setSelectedId] = useState(draftEmails[0]?.id ?? "");

  const selectedEmail = useMemo(
    () => draftEmails.find((mail) => mail.id === selectedId) ?? draftEmails[0],
    [selectedId, draftEmails]
  );
      ...
      <div className="mx-auto grid min-h-screen max-w-[1600px] grid-cols-1 lg:grid-cols-[260px_1fr_1.15fr]">
        ...
        {draftEmails.length === 0 ? (
          <section className="flex items-center justify-center bg-white lg:col-span-2">
            <p className="text-lg font-medium text-slate-500">No drafts pending review</p>
          </section>
        ) : (
          <>
            <section className="border-r border-slate-200 bg-white">
              <div className="border-b border-slate-200 px-6 py-5">
                ...
                <p className="text-sm text-slate-500">
                  {draftEmails.length} draft emails pending review
                </p>
              </div>
