...
export default function InboxPage() {
  ...
  const selectedEmail = useMemo(
    () => mockEmails.find((mail) => mail.id === selectedId) ?? mockEmails[0],
    [selectedId]
  );
  ...
  if (!selectedEmail) {
    return (
      <main className="min-h-screen bg-slate-100 p-10 text-slate-700">
      ...
      <div className="mx-auto grid min-h-screen max-w-[1600px] grid-cols-1 lg:grid-cols-[260px_1fr_1.15fr]">
        ...
        <section className="border-r border-slate-200 bg-white">
          <div className="border-b border-slate-200 px-6 py-5">
            ...
            <p className="text-sm text-slate-500">{mockEmails.length} messages requiring review</p>
          </div>
        ...
        <section className="bg-slate-50 px-6 py-6">
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            ...
            <span className={`rounded-full border px-3 py-1 text-xs font-medium ${statusColors[selectedEmail.status]}`}>
          ...
          <article className="mb-5 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            {selectedEmail.aiDraft ? (
              ...
              <p className="text-sm text-slate-500">No AI draft generated for this message. Manual intervention required.</p>
            )}
          </article>
